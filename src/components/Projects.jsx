  ...
  return (
    ...
    <div className="w-full h-11/12 mt-5 pointer-events-none flex items-start justify-center ">
      <Draggable nodeRef={nodeRef} bounds="parent" disabled={isTouchDevice}>
        ...
        className="w-[92vw] sm:w-3/4 md:w-2/3 h-[70vh] sm:h-[75vh] md:h-[80vh] mt-4 bg-black/50 backdrop-blur-lg rounded-2xl border border-gray-400/30 shadow-lg p-2 text-white flex items-start justify-between select-none pointer-events-auto">
          <div className='w-full h-full p-2 bg-gray-700/50 backdrop-blur-lg rounded-xl border border-gray-300/80 shadow-lg select-none flex flex-col items-start'> 
            <div className='w-full h-full flex flex-col'>
              <div className=" group w-3 h-3 bg-red-500 rounded-full mr-2 mb-2 flex items-center justify-center transform transition-transform ease-in-out hover:scale-125"
          ...
          <div ref={nodeRef} className='w-[92vw] sm:w-4/5 md:w-2/3 lg:w-1/2 h-[60vh] sm:h-[70vh] bg-[#c3c3c3] text-black border-3
...
const ProjectItem = ({ title, description, technologies, logo, githubLink, youtubeLink, isWindows }) => {
  return (
    <div className="flex flex-col">
      <p className={`${isWindows ? 'text-lg sm:text-xl font-semibold mb-2' : 'text-xl sm:text-2xl tracking-wide font-semibold mb-4'}`}>{title}</p>
      <div className='flex flex-col sm:flex-row sm:items-start'>
        <img
          src={logo}
          alt={`${title} Logo`}
          className={`${isWindows 
            ? 'w-28 h-28 sm:w-36 sm:h-36 md:w-40 md:h-40 sm:mr-4 mt-2 mb-3 sm:mb-0 select-none pointer-events-none' 
            : 'w-28 h-28 sm:w-36 sm:h-36 md:w-40 md:h-40 sm:mr-4 mt-1 mb-3 sm:mb-0 select-none pointer-events-none'}`} />
        <div>
          <p className={`${isWindows ? 'text-base sm:text-lg' : 'tracking-wide text-base sm:text-lg'}`}>{description}</p>
          <p className={`${isWindows ? 'text-base sm:text-lg my-1' : 'tracking-wide text-base sm:text-lg my-1'}`}>Technologies: {technologies}</p>
          <div className="flex gap-3 sm:gap-4">
            <img
              src={isWindows ? githubWin : githubLogo}
              alt="GitHub"
              className="w-10 h-10 sm:w-12 sm:h-12 rounded-full overflow-hidden mt-2 cursor-pointer"
              draggable="false"
